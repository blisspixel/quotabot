...
/// provider's current models and is regenerated by the catalog refresh tool
/// (`bin/refresh_catalog.dart`) from each provider's own model endpoint, so it
/// never has to be hand-maintained. Local-runtime models are NOT here; those are
/// read live from the runtime by the adapters.
///
/// Capability fields are hints (context window, tool use, vision, reasoning) and
/// may lag a provider's newest release until the next refresh; the registry
/// always pairs them with that provider's live quota for the budget half.
library;
