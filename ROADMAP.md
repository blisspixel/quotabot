...
| Accessibility and operator diagnostics | Partial | Desktop text scaling, keyboard and theme coverage, structured errors, provider-scoped `check`, contact-scoped strict `verify`, fail-closed empty live scope, `explain`, deterministic host-isolated simulation, and provenance-complete privacy-safe weekly reports exist | Run the final native keyboard/screen-reader smoke and verify every critical failure is actionable |
