...
| Accessibility and operator diagnostics | Partial | Desktop text scaling, keyboard and theme coverage, rendered-content quota sizing that stays inside the active work area, responsive display-bounded compact and Analytics sizing, visible scroll fallbacks, compact widget-order and assistive-action coverage, common quota-window and reset-time reflow at 200 percent text, structured errors, provider-scoped `check`, contact-scoped strict `verify`, fail-closed empty live scope, `explain`, deterministic host-isolated simulation, and provenance-complete privacy-safe weekly reports exist | Run the final native keyboard/screen-reader smoke and verify every critical failure is actionable |
